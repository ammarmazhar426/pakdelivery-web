...
import { Shield, Trash2, Plus, TrendingUp, MapPin, Package, Clock, AlertTriangle } from 'lucide-react'
...
export default function RiskIntel() {
  ...
  const handleCheck = async () => {
    ...
    try {
      const r = await axios.post(`${API}/orders/check-risk`, {
        name:'check', phone:checkPhone, address:'check', city:'check',
        product:'check', amount:100
      })
  ...
  const TABS = [
    ...
    { key:'heatmap',    label:'🗺️ Area Heatmap',  count: analytics?.city_heatmap?.length || 0 },
    { key:'products',   label:'📦 Products',      count: analytics?.product_risk?.length || 0 },
    { key:'checker',    label:'🔍 Phone Checker', count: null },
  ]
  ...
  return (
    <div style={{ padding:16, maxWidth:960, margin:'0 auto' }}>
      ...
      <div style={{ display:'flex', alignItems:'center', gap:12, marginBottom:20 }}>
        <div style={{ width:44, height:44, borderRadius:14, background:'rgba(244,63,94,0.15)', display:'flex', alignItems:'center', justifyContent:'center' }}>
          <Shield size={22} color="#F43F5E"/>
        </div>
        <div>
          <h1 style={{ fontSize:20, fontWeight:700, color:'var(--text-white)' }}>🤖 AI Risk Intelligence</h1>
          <p style={{ fontSize:11, color:'var(--text-dim)', marginTop:2 }}>
            Blacklist • Area Heatmap • Phone History • Smart Scoring
          </p>
        </div>
        {analytics && (
        ...
        {TABS.map(t => (
          <button key={t.key} onClick={() => setTab(t.key)} style={{
            ...
            {t.count !== null && t.count > 0 && (
              <span style={{ background: tab===t.key ? 'rgba(255,255,255,0.3)':'var(--glass2)', borderRadius:20, fontSize:9, padding:'1px 5px' }}>{t.count}</span>
            )}
          </button>
        ))}
      </div>
          ...
          <div style={{ background:'var(--glass)', border:'1px solid rgba(244,63,94,0.3)', borderRadius:16, padding:16, marginBottom:16 }}>
            ...
            <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:10, marginBottom:10 }}>
              <input placeholder="Phone Number *" value={phone} onChange={e=>setPhone(e.target.value)}
                style={{ padding:'9px 12px', background:'var(--glass2)', border:'1px solid var(--glass2)', borderRadius:10, color:'var(--text-white)', fontSize:13 }}/>
              <input placeholder="Customer Naam" value={name} onChange={e=>setName(e.target.value)}
                style={{ padding:'9px 12px', background:'var(--glass2)', border:'1px solid var(--glass2)', borderRadius:10, color:'var(--text-white)', fontSize:13 }}/>
            </div>
            <div style={{ display:'flex', gap:10 }}>
              <input placeholder="Reason (e.g. 3 baar RTO, phone nahi uthata)" value={reason} onChange={e=>setReason(e.target.value)}
                style={{ flex:1, padding:'9px 12px', background:'var(--glass2)', border:'1px solid var(--glass2)', borderRadius:10, color:'var(--text-white)', fontSize:13 }}/>
            ...
            <div key={i} style={{
              ...
              <div style={{ flex:1 }}>
                <div style={{ display:'flex', alignItems:'center', gap:8 }}>
                  <p style={{ fontSize:14, fontWeight:700, color:'#F43F5E' }}>{b.phone}</p>
                  {b.name && <p style={{ fontSize:12, color:'var(--text-mid)' }}>— {b.name}</p>}
                  <span style={{ background:'rgba(244,63,94,0.15)', color:'#F43F5E', fontSize:10, fontWeight:700, padding:'2px 8px', borderRadius:20 }}>
                    {b.times_blocked || 1}x blocked
                  </span>
                </div>
                <p style={{ fontSize:11, color:'var(--text-dim)', marginTop:2 }}>
                  {b.reason || 'No reason given'} • {b.added_on}
                </p>
              </div>
              <button onClick={() => handleRemove(b.phone)} style={{ background:'rgba(244,63,94,0.1)', border:'1px solid rgba(244,63,94,0.3)', borderRadius:8, color:'#F43F5E', padding:'6px 10px', cursor:'pointer' }}>
                <Trash2 size={14}/>
              </button>
            </div>
          ))}
        ...
        <div>
          <div style={{ background:'var(--glass)', border:'1px solid var(--glass2)', borderRadius:16, padding:16, marginBottom:16 }}>
            <div style={{ display:'flex', alignItems:'center', gap:8, marginBottom:4 }}>
              ...
              <p style={{ fontSize:14, fontWeight:700, color:'var(--text-white)' }}>City / Area RTO Heatmap</p>
            </div>
            <p style={{ fontSize:11, color:'var(--text-dim)' }}>Apne orders ke data se auto-calculate hota hai</p>
          </div>

          {!analytics || analytics.city_heatmap.length === 0 ? (
            <div style={{ textAlign:'center', padding:40, color:'var(--text-dim)' }}>
              <MapPin size={40} color="var(--text-dim)" style={{ marginBottom:12 }}/>
              <p>Kafi orders nahi hain abhi — jaise jaise orders aate jayenge heatmap ban jayega</p>
            </div>
            ...
            <div key={i} style={{
              ...
              <div style={{ width:42, height:42, borderRadius:12, background:RISK_BG[c.risk_level], display:'flex', alignItems:'center', justifyContent:'center', flexShrink:0 }}>
                <span style={{ fontSize:20 }}>{c.risk_level==='high'?'🔴':c.risk_level==='medium'?'🟡':'🟢'}</span>
              </div>
              <div style={{ flex:1 }}>
                <div style={{ display:'flex', alignItems:'center', gap:8 }}>
                  <p style={{ fontSize:14, fontWeight:700, color:'var(--text-white)' }}>{c.city}</p>
                  <span style={{ fontSize:11, fontWeight:700, color:RISK_COLOR[c.risk_level], background:RISK_BG[c.risk_level], padding:'2px 8px', borderRadius:20 }}>
                    {c.rto_rate}% RTO
                  </span>
                </div>
                <p style={{ fontSize:11, color:'var(--text-dim)', marginTop:2 }}>
              ...
              {/* Bar */}
              <div style={{ width:80, height:6, background:'var(--glass2)', borderRadius:3 }}>
        ...
        <div>
          ...
          {!analytics || analytics.product_risk.length === 0 ? (
            ...
            <div key={i} style={{
              ...
              <div style={{ width:38, height:38, borderRadius:10, background:RISK_BG[p.risk_level], display:'flex', alignItems:'center', justifyContent:'center', fontSize:18 }}>
              ...
              <div style={{ flex:1 }}>
                <div style={{ display:'flex', alignItems:'center', gap:8, flexWrap:'wrap' }}>
                  ...
                  <span style={{ fontSize:11, fontWeight:700, color:RISK_COLOR[p.risk_level], background:RISK_BG[p.risk_level], padding:'2px 8px', borderRadius:20 }}>
                    {p.rto_rate}% RTO
                  </span>
                </div>
      ...
      {tab === 'checker' && (
        <div>
          <div style={{ background:'var(--glass)', border:'1px solid var(--glass2)', borderRadius:16, padding:16, marginBottom:16 }}>
            <p style={{ fontSize:13, fontWeight:700, color:'var(--text-white)', marginBottom:12 }}>🔍 Phone Number Check Karein</p>
            ...
            <div style={{ display:'flex', gap:10 }}>
              <input placeholder="03XX-XXXXXXX" value={checkPhone} onChange={e=>setCheckPhone(e.target.value)}
                ...
                style={{ flex:1, padding:'10px 14px', background:'var(--glass2)', border:'1px solid var(--glass2)', borderRadius:10, color:'var(--text-white)', fontSize:14 }}/>
          ...
          {checkResult && (
            <div style={{ background:'var(--glass)', border:`2px solid ${checkResult.level==='LOW'?'#00C566':checkResult.level==='MEDIUM'?'#F59E0B':'#F43F5E'}`, borderRadius:16, overflow:'hidden' }}>
              {/* Score header */}
              <div style={{ background: checkResult.level==='LOW'?'#00C566':checkResult.level==='MEDIUM'?'#F59E0B':'#F43F5E', padding:'16px 20px', display:'flex', alignItems:'center', justifyContent:'space-between' }}>
                <div>
                ...
                <div style={{ textAlign:'right' }}>
                  <p style={{ color:'white', fontSize:22, fontWeight:800 }}>{checkResult.emoji} {checkResult.level}</p>
              ...
              {checkResult.details?.phone_history?.total_orders > 0 && (
                <div style={{ padding:'12px 16px', borderBottom:'1px solid var(--glass2)' }}>
                  ...
                  <div style={{ display:'grid', gridTemplateColumns:'repeat(4,1fr)', gap:8 }}>
                    {[
                      { label:'Total Orders', val:checkResult.details.phone_history.total_orders, color:'var(--text-white)' },
                  ...
                  {checkResult.details.phone_history.is_returning_customer && (
                    <p style={{ fontSize:11, color:'#60A5FA', marginTop:8 }}>
                      ℹ️ Yeh returning customer hai — history upar dikh raha hai
                    </p>
                  )}
                </div>
              )}
                ...
                <div style={{ padding:'12px 16px' }}>
                  ...
                  {checkResult.issues.map((issue,i) => (
                    <div key={i} style={{ display:'flex', alignItems:'flex-start', gap:8, marginBottom:6 }}>
                      <p style={{ fontSize:12, color:'var(--text-white)' }}>{issue}</p>
                    </div>
                  ))}
                </div>
              )}
